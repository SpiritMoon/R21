...
/**********************************
 *	gettimeofday1 instead  gettimeofday api.
 *
 *	gettimeofday1  dont depend system time,
 *	we use  uptime + absolute time for  the timer. 
 *	fix bug for  timer work abnormal
 *	when  systme time be changed .
 *
 *  by chenxq  20160728
 * *********************************/

static  int gettimeofday1(struct timeval *tv, struct timezone *tz)
{
	...
	//tv->tv_sec  =  tt.tv_sec +  1569631082;	
	return  0;
...
//	pthread_sigmask(SIG_BLOCK, &mask, &oldmask);
	...
	while (1)
	{
		 ...
		 */
		pthread_mutex_lock(&timerq.mutex);
		if(timerq.first == NULL){ /* peiwh modified at 111220, for fixing timer bug */
		...
		struct timeval		abs_now;  /* peiwh modified at 111220, for fixing timer bug */
...
int timer_init()
{
	...
	rv = pthread_condattr_setclock(&timerq.attr, CLOCK_MONOTONIC);  
	rv = pthread_cond_init(&timerq.cond, &timerq.attr);
	if (rv != 0) {
	...
	clock_gettime(CLOCK_MONOTONIC, &tt); 
	ts.tv_sec = tt.tv_sec + INIT_WAIT_TIME;
	...
	*/	
	if (pthread_detach(timerq.ticker) != 0)
	{
		printf("##########detach error!############\n");
	}
	pthread_mutex_unlock(&timerq.mutex);
...
int timer_add(long sec, long usec, void(*hndlr)(void *), void *hndlr_arg)
{
	...
	if (tmp->prev == NULL)
	{
		...
		tmp->in_use = 0;
		timer_start(&(timerq.first->timeout));
		//pthread_kill(timerq.ticker, SIGALRM); /* peiwh modified at 111220, for fixing timer bug */
	}
	else
	{
		app->prev = tmp->prev;
...
//	printf("now time : %s [%u]\n",nowReadable, now);
	...
	sprintf(cmd,"echo %s [%lu] >/tmp/echo_time",nowReadable, now);
	system(cmd);
