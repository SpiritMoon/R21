...
/* Generated by 'tcpdump -dd arp and ether[21]=0x1' */
static struct sock_filter arp_sock_filter_insns[] = {
	{ 0x28, 0, 0, 0x0000000c },
	{ 0x15, 0, 3, 0x00000806 },
	{ 0x30, 0, 0, 0x00000015 },
	{ 0x15, 0, 1, 0x00000001 },
...
static const struct sock_fprog arp_sock_filter = {
	.len = ARRAY_SIZE(arp_sock_filter_insns),
	.filter = arp_sock_filter_insns,
};
		   ...
		   const u8 *buf, size_t len)
{
	...
	if (l2->l2_hdr) {
		...
		if (ret < 0)
			arpp_printf(ARPP_ERROR, "l2_packet_send - send: %s",
				   strerror(errno));
		else // zhaoyang1 modifies for hostap debug 2015-04-03
			arpp_printf(ARPP_DEBUG, "%s send packet successfully\n", __func__);
	} else {
		...
		ret = sendto(l2->fd, buf, len, 0, (struct sockaddr *) &ll,
			     sizeof(ll));
		if (ret < 0) {
			arpp_printf(ARPP_ERROR, "l2_packet_send - sendto: %s",
				   strerror(errno));
		} else // zhaoyang1 modifies for hostap debug 2015-04-03
			arpp_printf(ARPP_DEBUG, "%s send eapol packet successfully\n", __func__);
	}
	return ret;
}
...
static void l2_packet_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	res = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr *) &ll,
		       &fromlen);
	if (res < 0) {
		arpp_printf(ARPP_DEBUG, "l2_packet_receive - recvfrom: %s",
			   strerror(errno));
		return;
	}
	...
	void *rx_callback_ctx, int l2_hdr)
{
	...
	l2->fd = socket(PF_PACKET, l2_hdr ? SOCK_RAW : SOCK_DGRAM,
			htons(protocol));
	if (l2->fd < 0) {
		arpp_printf(ARPP_ERROR, "%s: socket(PF_PACKET): %s",
	...
	if (ioctl(l2->fd, SIOCGIFINDEX, &ifr) < 0) {
		arpp_printf(ARPP_ERROR, "%s: ioctl[SIOCGIFINDEX]: %s",
			   __func__, strerror(errno));
		close(l2->fd);
		os_free(l2);
		return NULL;
	...
	arpp_printf(ARPP_DEBUG, "%s, ifname: %s ifindex: %d raw fd: %d protocol: 0x%x\n",
			__func__, l2->ifname, l2->ifindex, l2->fd, protocol);
	
	if (bind(l2->fd, (struct sockaddr *) &ll, sizeof(ll)) < 0) {
		arpp_printf(ARPP_ERROR, "%s: bind[PF_PACKET]: %s",
			   __func__, strerror(errno));
		close(l2->fd);
		os_free(l2);
		return NULL;
	...
	if (ioctl(l2->fd, SIOCGIFHWADDR, &ifr) < 0) {
		arpp_printf(ARPP_ERROR, "%s: ioctl[SIOCGIFHWADDR]: %s",
			   __func__, strerror(errno));
		close(l2->fd);
		os_free(l2);
		return NULL;
...
void l2_packet_deinit(struct l2_packet_data *l2)
{
	...
	arpp_printf(ARPP_DEBUG, "%s, ifname: %s ifindex: %d raw fd: %d\n",
			__func__, l2->ifname, l2->ifindex, l2->fd);
...
int l2_packet_get_ip_addr(struct l2_packet_data *l2, char *buf, size_t len)
{
	...
	if (s < 0) {
		arpp_printf(ARPP_ERROR, "%s: socket: %s",
			   __func__, strerror(errno));
		return -1;
	}
	os_memset(&ifr, 0, sizeof(ifr));
	...
	if (ioctl(s, SIOCGIFADDR, &ifr) < 0) {
		if (errno != EADDRNOTAVAIL)
			arpp_printf(ARPP_ERROR, "%s: ioctl[SIOCGIFADDR]: %s",
				   __func__, strerror(errno));
		close(s);
		return -1;
	}
				...
				enum l2_packet_filter_type type)
{
	...
	switch (type) {
	case L2_PACKET_FILTER_ARP:
		sock_filter = &arp_sock_filter;
		break;
