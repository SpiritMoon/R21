...
pushuser(struct apuser *user, bool init, char *name)
{
	...
	um_user_add_u32(UM_USER_MODE, user->wifi.mode);
	...
	um_user_add_u32(UM_USER_UP_RATE, user->wifi.rx.uprate);
	um_user_add_u32(UM_USER_DOWN_RATE, user->wifi.tx.downrate);
...
setfilter_mac(struct user_filter *filter, struct blob_attr *attr[])
{
	...
	if (p) {
	    ...
	    int len = os_strlen(blobmsg_get_string(p));
	    if (17 != len && 12 != len)
	    {
            return false;
	    }
...
add_object(struct ubus_object *obj)
{
	...
	if (err) {
		debug_ubus_waring("[um]:Failed to publish object '%s': %s\n", obj->name, ubus_strerror(err));
	}
...
add_subscriber(struct ubus_subscriber *subscriber)
{
	...
	if (err) {
		debug_ubus_waring("[um]:Failed to register subscriber(%s)", ubus_strerror(err));
	}
...
um_ubus_connect(char *path)
{
	...
	if (NULL==umc.ctx) {
	    debug_ubus_waring("[um]:connect ubus failed");
