...
hostapd_set_bss_options() {
	...
	config_get wpa_group_rekey  "$vif" wpa_group_rekey 0
	...
	[ -f /var/run/wam-$device/$ifname ] && rm /var/run/wam-$device/$ifname
	ctrl_interface=/var/run/wam-$device
	...
	if [ "$wpa" -ge "2" ]
	then
		#Xi Kai add OKC function
		config_get okc "$vif" okc 0
		case "$enc" in
			*wpa*)
			if [ "$okc" = 1 ]
			then
...
hostapd_setup_vif() {
	...
	local no_nconfig
	local device channel hwmode

	date >> /tmp/log/wam_activity
	echo "wam_setup_vif $ifname $vif $driver"  >> /tmp/log/wam_activity
	...
	cat > /var/run/wam-$ifname.conf <<EOF
driver=$driver
interface=$ifname
...
EOF
	[ -z "${no_nconfig}" ] &&
		echo ${hwmode:+hw_mode=${hwmode#11}} >> /var/run/wam-$ifname.conf
	...
	if [ $wps_vap_tie_dbdc -ne 0 ]; then
		echo -e "/var/run/wam-$ifname.conf \c\h" >> /tmp/hostapd_conf_filename
	else
		if [ $ifname != "athscan0" -a $ifname != "athscan1" ];then
			hostapd_check_process "$ifname"
			wam -P /var/run/wifi-$ifname.pid -B /var/run/wam-$ifname.conf -d -f /tmp/log/wam-$ifname.log
			echo -e $ifname >> /tmp/run/wam_ifname_file
		fi
		# zhaoyang1 modifies for binding sock with wlan interface 2015-04-03
		if [ -n "$wps_possible" -a -n "$config_methods" ]; then
			pid=/var/run/wam_cli-$ifname.pid
			wam_cli -i $ifname -P $pid -a /lib/wifi/wps-hostapd-update-uci -p /var/run/wam-$device -B
		fi
	fi
...
hostapd_remove_vif() {                                                                
        ...
        date >> /tmp/log/wam_activity                                                                               
        echo "wam_remove_vif $ifname"  >> /tmp/log/wam_activity                                                     
                                                                                                                    
        sed -i '/'"$ifname"'/d' /tmp/run/wam_ifname_file                                                                                     
                                                                                                                                             
        if [ -f "/var/run/wifi-${ifname}.pid" ];then                                                                 
                echo "kill /var/run/wifi-${ifname}.pid $(cat "/var/run/wifi-${ifname}.pid")" >> /tmp/log/wam_activity
                kill "$(cat "/var/run/wifi-${ifname}.pid")"                                                      
		rm "/var/run/wifi-${ifname}.pid"
        fi                                                                                                       
	...
	sleep 0.01
        ...
        while [ $t -gt 0 ]                                                            
        do                                                                                                   
                ps -w | grep "wam -P /var/run/wifi-$ifname.pid -B /var/run/wam-$ifname.conf" | grep -v "grep"                                
                if [ $? -eq 0 ];then                                                                                                         
                        local pid=`ps -w | grep "wam -P /var/run/wifi-$ifname.pid -B /var/run/wam-$ifname.conf" | grep -v "grep" | awk '{print $1}'`
                        if [ $pid -gt 0 ];then                                      
                                kill -9 $pid                                        
                                echo "kill $pid" >> /tmp/log/wam_activity    
                        fi                                                                                       
                else                                                                                             
                        break                                                         
                fi                                                                    
...
hostapd_check_process() {
	local ifname="$1" && shift
	
	grep "$ifname" /tmp/run/wam_ifname_file
	if [ $? -eq 0 ];then
		sed -i '/'"$ifname"'/d' /tmp/run/wam_ifname_file
