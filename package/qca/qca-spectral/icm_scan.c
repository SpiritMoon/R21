            ...
            if (extension_channel) {
                  for(i = 0; i < pchlist->count; i++) {
                        if (pchlist->ch[i].channel == extension_channel) {
                            ...
                            printf("ch[%d] used_as_secondary_20 %d\n",i,pchlist->ch[i].used_as_secondary_20);
                            break;
                        }
                  }
            } /* End of If */
...
int icm_do_80211_scan(ICM_INFO_T * picm)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"icm_do_80211_scan\n");
    ...
    if(os_strcmp(picm->radio_ifname, "wifi0") == 0){  
        system("bg-s -x fastscan=athscan0");
        printf("%s set fast scanning in athscan0\n",__func__);
    }else if(os_strcmp(picm->radio_ifname, "wifi1") == 0){ 
        system("bg-s -x fastscan=athscan1");
        printf("%s set fast scanning in athscan1\n",__func__);
    }
    #endif
    system("bg-s -x pause_scanning=1");
    
    drm_log(DRM_LOG_NORMAL,"ACS:call scanning ....................\n");
    while((ioctl(pnlinfo->sock_fd, SIOCSIWSCAN, &wrq) < 0)){
        if(scan_retry_cnt > 5){
        ...
        drm_log(DRM_LOG_NORMAL,"ACS:scan busy ......\n");
        usleep(10000);
    }
    #if 0
        ...
        if (ret == 0) {
          ...
          realloc:
            ...
            if (newbuf == NULL) {
                ...
                fprintf(stderr, "%s: Allocation failed realloc len is %d\n", __func__,buflen);
                picm->substate = ICM_STATE_INVALID;
                icm_restore_scanband(pdev, picm);
                icm_disable_11ac_scanchevents(pdev, picm, is_11ac);
                icm_disable_sending_additional_ies(pdev, picm);
            ...
            #if 0
            if(os_strcmp(picm->radio_ifname, "wifi0") == 0){  
                strncpy(wrq.ifr_name, "athscan0", 8);
                printf("%s get scanning result from %s\n",__func__,wrq.ifr_name);
            }else if(os_strcmp(picm->radio_ifname, "wifi1") == 0){ 
                strncpy(wrq.ifr_name,  "athscan1", 8);
                printf("%s get scanning result from %s\n",__func__,wrq.ifr_name);
            }
            #endif
            wrq.ifr_name[IFNAMSIZ - 1] = '\0';
...
void icm_update_wnw_in_channel_list(ICM_INFO_T* picm, ICM_BAND_T band)
{
    ...
    for (i = 0; i < pchlist->count; i++) {
        ...
        printf("%s channel[%d] ap_num is %d\n",__func__,i,pchlist->ch[i].num_wnw);
    }
