...
#define OFFCHAN_EXT_TID_NONPAUSE    19
    ...
    /* For 11ac statistics, Yuyaowen added. */
#if ATOPT_ORI_ATHEROS_BUG_11AC_COUNT
    ni = ieee80211_vap_find_node(vap, eh->ether_dhost);
    if (ni) {
        if (IEEE80211_IS_MULTICAST(eh->ether_dhost)) {
            ...
            mac_stats = &vap->iv_unicast_stats;
        }
        ni->ni_stats.ns_tx_data++;
        ni->ni_stats.ns_tx_bytes += skb->data_len;
        ieee80211_free_node(ni);
    }

    mac_stats->ims_tx_packets++;
    mac_stats->ims_tx_bytes += skb->data_len;
    mac_stats->ims_tx_data_packets++;
    mac_stats->ims_tx_data_bytes += skb->data_len;
#endif /* ATOPT_ORI_ATHEROS_BUG_11AC_COUNT */
    /* End 11ac statistics */
...
osif_ol_ll_vap_hardstart(struct sk_buff *skb, struct net_device *dev)
{
    ...
    */
    ...
    mac_stats->ims_tx_packets++;
    mac_stats->ims_tx_bytes += skb->len;
    mac_stats->ims_tx_data_packets++;
    mac_stats->ims_tx_data_bytes += skb->len;
#endif /* ATOPT_ORI_ATHEROS_BUG_11AC_COUNT */
    /* End: For 11ac statistics */
...
// fixme where is this done in the new scheme?    dev->priv = vap;
    ...
    */
    ...
    #if ATOPT_ORI_ATHEROS_BUG
    printk(KERN_INFO "VAP device %s created osifp: (%p) os_if: (%p)\n",
