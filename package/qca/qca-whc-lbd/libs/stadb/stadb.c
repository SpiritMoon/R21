...
/*Begin:pengdecai add for dcm*/
extern unsigned char  
han_get_rssi_threshold_form_bss(const lbd_bssInfo_t *bss) ;
extern unsigned char
han_dcm_get_rssi(stadbEntry_handle_t entry);
static void han_kickout_lowrssi_staions(stadbEntry_handle_t entry, void *cookie)
{
   ...
   if(entry->assoc.bssHandle && entry->activity.isAct){
	   ...
	   const lbd_bssInfo_t *bssInfo = stadbEntry_resolveBSSInfo(bssHandle);
          rssiThreshold = han_get_rssi_threshold_form_bss(bssInfo);
	    if(!rssiThreshold) 
			return;
	    ...
	    if(rssi > 0 && rssi < rssiThreshold){
   			//add blacklist 
   			//kickout sta	
   		  wlanif_disassociateSTA(bssInfo,&entry->addr);
	    }
   }
    return;	
}

void han_dcm_kickout_lowrssi_staions(void)
{
   stadb_iterate(han_kickout_lowrssi_staions, NULL);
}

/*End:pengdecai add for dcm*/
