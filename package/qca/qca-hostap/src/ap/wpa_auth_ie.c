			...
			const u8 *mdie, size_t mdie_len)
{
	...
	for (i = 0; sm->pmksa == NULL && wpa_auth->conf.okc &&
		...
		idata.pmksa = pmksa_cache_get_okc(wpa_auth->pmksa, idata.aa, idata.spa,
					  idata.pmkid);
		if (idata.pmksa) {
			...
			sm->pmksa = os_zalloc(sizeof(struct rsn_pmksa_cache_entry));
	        if (sm->pmksa == NULL)
		        return WPA_ALLOC_FAIL;
	        os_memcpy(sm->pmksa->pmkid, idata.pmkid, PMKID_LEN);
	        os_memcpy(sm->pmksa->pmk, idata.pmksa->pmk, idata.pmksa->pmk_len);
	        sm->pmksa->pmk_len = idata.pmksa->pmk_len;
	        sm->pmksa->expiration = idata.pmksa ->expiration;
	        sm->pmksa->akmp = idata.pmksa->akmp;
	        os_memcpy(sm->pmksa->spa, idata.pmksa->spa, ETH_ALEN);
			pmkid = idata.pmkid;
			break;
		}
	}
