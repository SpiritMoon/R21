...
LOGHEAD="logger -t [common.sh] -p 2 "
...
run_ramfs() { # <command> [...]
	...
	supivot $RAM_ROOT /mnt || {
        ...
        $LOGHEAD "=upgrade= Failed to switch over to ramfs."
		exit 1
	}
...
kill_remaining() { # [ <signal> ]
    ...
    $LOGHEAD "=upgrade= Sending $sig to remaining processes ..."
	...
	for stat in /proc/[0-9]*/stat; do
		...
		if [ $$ -eq 1 ] || [ $my_ppid -eq 1 ] && [ -n "$my_ppisupgraded" ]; then
			...
			case "$name" in
				...
				*procd*|*ash*|*init*|*watchdog*|*ssh*|*dropbear*|*telnet*|*login*|*hostapd*|*wpa_supplicant*|*nas*|*upgrade*) : ;;
...
do_upgrade() {
    ...
    $LOGHEAD "=upgrade= performing system upgrade..."
	...
	v "Upgrade completed"
    $LOGHEAD "=upgrade= upgrade completed"
	[ -n "$DELAY" ] && sleep "$DELAY"
	ask_bool 1 "Reboot" && {
		v "Rebooting system..."
        $LOGHEAD "=reboot= reboot after upgrade completed"
		/usr/sbin/reset_reason add 05
		reboot -f
		sleep 5
