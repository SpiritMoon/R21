...
static ssize_t kes_debug_flag_proc_write(struct file *flip, const char __user *buff, size_t len, loff_t *ppos)
{
    ...
    if(len > 3) //just a character and a '\0' is enough
    {
    ...
    memcpy(kes_debug_header->isenable,tmp,KES_ISENABLE_LEN);
...
static int kes_debug_flag_proc_read(struct seq_file *m, void *v)
{
    ...
    memcpy(tmp, kes_debug_header->isenable, KES_ISENABLE_LEN);
...
int print_msg_to_kes_debug(const char * fmt,...)
{
    ...
    {
        memset(kes_debug_addr, 0, KES_DEBUG_BLOCK_SIZE);
        memcpy(kes_debug_header->magic, "kes-log", KES_MAGIC_LEN);
...
int kes_debug_init(long unsigned int kes_mem_addr_l)
{
    ...
    /* get kes_debug_addr */
    pfn = (kes_mem_addr_l + KES_TRAPS_BLOCK_SIZE) >> PAGE_SHIFT;
