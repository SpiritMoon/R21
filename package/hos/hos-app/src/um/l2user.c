...
#define LIST_STATION_ALLOC_SIZE 30*1024  
#define MAC80211
...
l2user_update(bool created, struct apuser *dst, struct apuser *src)
{
    ...
    */
...
#if 0
	uprate = (src->wifi.rx.bytes- dst->wifi.rx.bytes) / 1024 / 5;	/* 5s kB/s */
	downrate = (src->wifi.tx.bytes - dst->wifi.tx.bytes) / 1024 / 5;	/* 5s kB/s */
#endif 
    if (src->wifi.rx.bytes > dst->wifi.rx.bytes)
	{
		uprate = (src->wifi.rx.bytes - dst->wifi.rx.bytes) / 1024 / 5;	/* 5s kB/s */
    ...
    else if (src->wifi.rx.bytes == dst->wifi.rx.bytes)
	{
		uprate = 0;
	}
    else
	{
		uprate = dst->wifi.rx.bytes / 1024 / 5; /* 5s kB/s */
	}
	
    if (src->wifi.tx.bytes > dst->wifi.tx.bytes)
	{
		downrate = (src->wifi.tx.bytes - dst->wifi.tx.bytes) / 1024 / 5;		/* 5s kB/s */
   ...
   else if (src->wifi.tx.bytes == dst->wifi.tx.bytes)
	{
		downrate = 0;
        }
   else
	{
		downrate = dst->wifi.tx.bytes / 1024 / 5;		/* 5s kB/s */
	}

    os_objdcpy(&dst->wifi.rx, &src->wifi.rx);
    os_objdcpy(&dst->wifi.tx, &src->wifi.tx);

	dst->wifi.rx.uprate = uprate;
	dst->wifi.tx.downrate = downrate;
    
    dst->wifi.signal = src->wifi.signal;
	dst->wifi.mode= src->wifi.mode;
...
char*  get_ap_macaddr(void)
{
	static char ap_mac[20];
	int sockfd = -1;
	struct ifreq ifr;

	sockfd = socket(AF_INET, SOCK_DGRAM, 0);
	if (sockfd < 0){
		debug_l2timer_waring("[um]: create socket error for get_ap_macaddr");
		return NULL;
	}

	memset(&ifr, 0, sizeof(ifr));

	sprintf(ifr.ifr_name, "br-wan");

	if (ioctl(sockfd, SIOCGIFHWADDR, &ifr) < 0){
		debug_l2timer_waring("[um]: ioctl failed for get_ap_macaddr");
		close(sockfd);
	}
	...
	debug_l2timer_trace("[um]: ap_mac:[%s]\n", ap_mac);
...
char* get_vap_macaddr(char * ifname)
{
	static char vap_mac[20];
	int sockfd = -1;
	struct ifreq ifr;

	sockfd = socket(AF_INET, SOCK_DGRAM, 0);
	if (sockfd < 0){
		debug_l2timer_waring("[um]: create socket error for get_vap_macaddr");
		return NULL;
	}
	...
	if (ioctl(sockfd, SIOCGIFHWADDR, &ifr) < 0){
		debug_l2timer_waring("[um]: ioctl failed for get_vap_macaddr");
		close(sockfd);
	}
	sprintf(vap_mac, "%02x:%02x:%02x:%02x:%02x:%02x",
		(unsigned char)ifr.ifr_netmask.sa_data[0],
		(unsigned char)ifr.ifr_netmask.sa_data[1],
		(unsigned char)ifr.ifr_netmask.sa_data[2],
		(unsigned char)ifr.ifr_netmask.sa_data[3],
		(unsigned char)ifr.ifr_netmask.sa_data[4],
		(unsigned char)ifr.ifr_netmask.sa_data[5]);
	debug_l2timer_trace("[um]: vap_mac:[%s]\n", vap_mac);
...
static unsigned int l2user_wifirate(char *ifname)
{
	...
	sockfd = socket(AF_INET, SOCK_DGRAM, 0);
	if (sockfd < 0){
		debug_l2timer_waring("[um]: create socket failed for get wifi rate");
		return -1;
	}
	...
	if (ioctl(sockfd, SIOCGIWRATE, &iwr) < 0){
		debug_l2timer_waring("[um]: ioctl failed for get wifi rate");
		close(sockfd);
		return -1;
	}

	rate = iwr.u.bitrate.value;
	debug_l2timer_trace("[um]: Rate:[%d]\n", iwr.u.bitrate.value);
...
static int l2user_timer(struct um_intf *intf)
{
	...
	if (ioctl(s, IEEE80211_IOCTL_STA_INFO, &iwr) < 0){
		debug_l2timer_waring("[um]: unable to get station information in [%s]", iwr.ifr_name);
        close(s);
	...
	info.wifi.rx.wifirate = info.wifi.tx.wifirate;
	do {
