...
time_t g_spectral_time;
unsigned int g_spectral_alarm_time = 0;
int g_timeout_random_value;
...
void signal_handler(int signals)
{
    ...
    switch(signals) {
        ...
        case SIGALRM:
            alarm_handler(pdev);
            drm_config_check_timer();
            acs_control_state_machine_timer();
            atp_control_timer();
			/*
...
void alarm_handler(ICM_DEV_INFO_T *pdev)
{
    ICM_INFO_T* picm = NULL;
    time_t now_time;
    if(g_spectral_alarm_time == 0)
        return;
 
    time(&now_time);
    
    if(g_spectral_time == NULL){
		printf("Get first check time\n");
		time(&g_spectral_time);
        return;
	}else{ 
	    if((now_time - g_spectral_time) < g_spectral_alarm_time){
	        return;
	    }
    }
    /*disable the timer */
    ...
    if (picm->sinfo.spectral_scan_count < MAX_NUM_SPECTRAL_SCAN_COUNT) {
        ...
        if (picm->current_channel == 1) {
            ...
            g_spectral_alarm_time = picm->sinfo.dwell_interval_short;
        } else {
            //alarm(picm->sinfo.dwell_interval);
            g_spectral_alarm_time = picm->sinfo.dwell_interval;
        }
    } else {
        icm_stop_spectral_scan(picm);
		...
		drm_log(DRM_LOG_WRITE_TO_SYSLOG,"alarm handler:send ICM_EVENT_SPECTRAL_SWEEP_DONE to SPECTRAL_LOOP\n");
...
int icm_init(ICM_DEV_INFO_T* pdev)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"radio num=%d\n",conf->num_radios);
    ...
    for (i = 0; i < conf->num_radios; i++) {
        ...
        picm->usage_factor_80_ppdu = DEFAULT_USAGE_FACTOR_80_PPDU;
		/*
		 *printf("pre dev_ifname %s,radion_ifname=%s,band is %d, channel width is %d\n",
		 *        picm->dev_ifname,
		 *        picm->radio_ifname,
		 *        picm->band,picm->channel_width);
		 */
		...
		/*
		 *printf("after dev_ifname %s,radion_ifname=%s,band is %d, channel width is %d\n",
		 *        picm->dev_ifname,
		 *        picm->radio_ifname,
		 *        picm->band,picm->channel_width);
		 */
        /*acs control read config*/
...
static bool icm_is_vap_radiochild(const u_int8_t *vapaddr,
                                  const u_int8_t *radioaddr)
{
    if (!memcmp(vapaddr, radioaddr, 5)) {
        if(vapaddr[5] >= radioaddr[5]){
            if(vapaddr[5] - radioaddr[5] <= 7)
                return TRUE;
        }
    } else {
        return FALSE;
...
int main(int argc, char* argv[])
{
    ...
    openlog("[DRM-LOG]",0,LOG_DAEMON);
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"version 1.1.2.3\n");
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"check if aps are ready,please waiting...\n");
	printf("check if aps are ready,please waiting...\n");
	sleep(10);
	wait_prerequisite_of_vap();
	srand((unsigned)time(NULL));
	g_timeout_random_value = rand() % 16;
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"g_timeout_random_value =%d\n",g_timeout_random_value);
    ...
    for (;;) {
        read_fds = master;
        ret = select(fdmax + 1, &read_fds, NULL, NULL, 0);
        if (ret < 0) {
...
int icm_set_width_and_channel(ICM_INFO_T *picm, char *dev_ifname)
{
    ...
    if (channel_width > ICM_CH_BW_80) {
        err("Unsupported channel width enumeration %u", channel_width);
        return FAILURE;
    }
    ...
    acs_store_wlan_mode(picm,modestr);
    ret = system(cmd);
...
static int icm_init_radio_iface(ICM_INFO_T* picm, char *radio_ifname)
{
    ...
    for (i = 0; i < vap_listing.curr_size; i++ )
    {
        ...
        if (icm_is_vap_radiochild(vapaddr, radio_addr)) {
            ...
            numdevs++;
            drm_get_channel_txpower(picm,(char *)vap_listing.ifnames[i]);
			...
			/*
			 *printf("vap list %d,ifname %s\n",numdevs,vap_listing.ifnames[i]);
			 */
	...
	if(picm->numdevs > 0) {
        ...
        picm->def_channel = 0;//icm_get_radio_default_channel(picm->dev_ifname);
		/*ACS control*/
