...
static void wlanifLinkEventsProcessChannelChange(
    ...
    if (newBand != band) {
        dbgf(state->dbgModule, DBGERR,
             ...
             __func__);
	 printf("%s: Change of band not supported in lbd; restarting",
             __func__);
    ...
    if (wlanifBSteerControlUpdateChannel(state->bsteerControlHandle,
         ...
         dbgf(state->dbgModule, DBGERR,
              ...
              __func__);
		  printf("%s: Could not update channel in lbd; restarting",
              __func__);
		wlanifBSteerControlDisable(state->bsteerControlHandle);
         exit(1);
    }
...
static void wlanifLinkEventsRestartUtilization(
    ...
    if (wlanifBSteerControlRestartChannelUtilizationMonitoring(
	   ...
	   wlanifBSteerControlDisable(state->bsteerControlHandle);
        ...
        dbgf(state->dbgModule, DBGERR,
             "%s: Successfull to restart utilization monitoring; "
            , __func__);
    }
...
static void wlanifLinkEventsGenerateDisassocEvent(
	...
	int i ;
	for (i = 0;i < 3; i ++){
		if(g_own_ap_state.radio[i].channelID == assocEvent.bss.channelId){
			if(g_own_ap_state.radio[i].stanum)
			g_own_ap_state.radio[i].stanum --;
		}
	}
#endif
}
...
static void han_handle_iface_state_change(wlanifLinkEventsHandle_t state,
                                          ...
                                          u_int32_t payloadLen)
{
   ...
   #if 0
     printf("%s ifaceInfo->ifi_change = 0x%x;ifaceInfo->ifi_flags=0x%x \n",__func__,ifaceInfo->ifi_change,ifaceInfo->ifi_flags);
   
     if((ifaceInfo->ifi_flags & IFF_RUNNING)){
	    printf("%s ifi_flags IFF_RUNNING  \n",__func__);
     }
     if((ifaceInfo->ifi_flags & IFF_UP)){
	    printf("%s ifi_flags IFF_UP  \n",__func__);
     }
   #endif
   
      if((ifaceInfo->ifi_flags & IFF_UP)){
	    printf("%s ineterface is now UP  ifi_index= %d  \n",__func__,ifaceInfo->ifi_index);
     }else {
	    printf("%s ineterface is now Down ifi_index= %d  \n",__func__,ifaceInfo->ifi_index);
     }
    if((ifaceInfo->ifi_flags &  IFF_RUNNING)){
	    printf("%s han check and add vap ifinex = %d \n",__func__,ifaceInfo->ifi_index);

             //check && add
          if(LBD_NOK == han_check_and_add_vap(state->bsteerControlHandle, ifaceInfo->ifi_index)){
	 		  printf("%s han check and add vap error ifinex = %d \n",__func__,ifaceInfo->ifi_index);
          }
    }else if((ifaceInfo->ifi_flags &  IFF_RUNNING) == 0) {
    		printf("%s han vap stop  ifindex = %d\n",__func__,ifaceInfo->ifi_index);

		if(han_vap_set_stop(state->bsteerControlHandle,ifaceInfo->ifi_index)){
			printf("%s han vap stop error ifindex = %d\n",__func__,ifaceInfo->ifi_index);
		}
...
static void wlanifLinkEventsHandleNewLink(wlanifLinkEventsHandle_t state,
    ...
    /*pengdecai for dcm*/
    han_handle_iface_state_change(state,hdr,payloadLen);
