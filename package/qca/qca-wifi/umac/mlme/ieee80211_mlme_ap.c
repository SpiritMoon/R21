         ...
         if ( !ic->ic_is_mode_offload(ic) && !IEEE80211_VAP_IS_PRIVACY_ENABLED(vap)  &&  (ni->ni_ucastkey.wk_keyix == IEEE80211_KEYIX_NONE) && !(vap->iv_wps_mode) ) {
           if (!RSN_AUTH_IS_WPA(&ni->ni_rsn) &&
                ...
                !RSN_AUTH_IS_8021X(&ni->ni_rsn)) {
					...
					if(flag){ //pengdecai modified for panic when assoc 125 stations.
...
/*
 * Function: _dedupauth
 * Description: deduplication station auth
 * Author: Yuyaowen
 * Date: 2016-06-17
 */
#if ATOPT_ORI_ATHEROS_BUG_DEDUP_AUTH
static void
_dedup_auth(struct ieee80211_node *ni, u_int16_t reason_code)
{
#define NR_OF_WIFI 3   // Numbers of wifi

	int authed_ic_index = 0;
	int i = 0;
	static struct ieee80211com *authed_ic_table[NR_OF_WIFI] = {NULL};
	struct ieee80211com *cur_ic;
	struct ieee80211_node *dup_ni[NR_OF_WIFI] = {NULL};
	struct ieee80211vap *vap = ni->ni_vap;

	cur_ic = ni->ni_ic;

	while (authed_ic_index < NR_OF_WIFI) {  // create authed ic set
		if (authed_ic_table[authed_ic_index] != cur_ic) {
			if (authed_ic_table[authed_ic_index] == NULL) {
				authed_ic_table[authed_ic_index] = cur_ic;
				break;
			}
			authed_ic_index++;
			continue;
	...
	authed_ic_index = 0;
	while (authed_ic_index < NR_OF_WIFI) {
		if ((authed_ic_table[authed_ic_index] != cur_ic) && (authed_ic_table[authed_ic_index] != NULL)) {
			dup_ni[i] = ieee80211_find_node(&authed_ic_table[authed_ic_index]->ic_sta, ni->ni_macaddr);
			ieee80211_free_node(dup_ni[i]);
			i++;
		}
		authed_ic_index++;
	}
	...
	i = 0;
	while (i < (NR_OF_WIFI - 1)) {
		if (dup_ni[i] != NULL) {  // found authed other wifi
			vap = dup_ni[i]->ni_vap;
			if (dup_ni[i] != vap->iv_bss) {
#if ATH_SUPPORT_AOW
				ieee80211_aow_join_indicate(dup_ni[i]->ni_ic, AOW_STA_DISCONNECTED, dup_ni[i]);
#endif  /* ATH_SUPPORT_AOW */
				ieee80211_ref_node(dup_ni[i]);
				if(IEEE80211_NODE_LEAVE(dup_ni[i])) {
				#if ATOPT_IGMP_SNP
				   /*Begin:pengdecai for han igmpsnp*/
					if(vap->iv_me->mc_snoop_enable){
						send_igmp_snooping_sta_leave(dup_ni[i]);
					}
					/*End:pengdecai for han igmpsnp*/
				 #endif
					/* Call MLME indication handler if node is in associated state */
					IEEE80211_DELIVER_EVENT_MLME_DEAUTH_INDICATION(vap, dup_ni[i]->ni_macaddr, reason_code);
				}
				ieee80211_free_node(dup_ni[i]);
			}
		}
...
#endif /* ATOPT_ORI_ATHEROS_BUG_DEDUP_AUTH */
                       ...
                       const struct ieee80211_rx_status *rs)
{
    ...
    /* For deduplication station auth. Yuyaowen added */
#if ATOPT_ORI_ATHEROS_BUG_DEDUP_AUTH
    u_int16_t reason;
    u_int8_t *frm;
#endif
    /* End for deduplication station auth. 20160621 */
    ...
    do {
        ...
        if (!ieee80211_acl_check(vap, wh->i_addr2)) {
            ...
            printk(KERN_NOTICE"[%s] auth: disallowed by ACL \n",ether_sprintf(wh->i_addr2));//Added by Yuyaowen.
		 ...
		 * pengdecai for dcm.
		 */
		if (!han_dcm_check(vap, wh->i_addr2)) {
			IEEE80211_DPRINTF(vap, IEEE80211_MSG_AUTH,
							  "[%s] auth: disallowed by dcm \n",ether_sprintf(wh->i_addr2));
