...
static int hostapd_ctrl_iface_dhcp_unicast(struct hostapd_data *hapd, char *cmd)
{
	int res;

	wpa_printf(MSG_DEBUG, "CTRL_IFACE DHCP_UNICAST '%s'", cmd);

	if (os_strcmp(cmd, "enable") == 0) {
		hapd->conf->dhcp_unicast = 1;
		return 0;
	}
	else if (os_strcmp(cmd, "disable") == 0) {
		hapd->conf->dhcp_unicast = 0;
		return 0;
	}
				       ...
				       void *sock_ctx)
{
	...
	} else if(os_strncmp(buf, "DHCP_UNICAST ",13) == 0) {
		reply_len = hostapd_ctrl_iface_dhcp_unicast(hapd, buf + 13);
	} else {
#ifdef CONFIG_HS20
		if (hostapd_ctrl_iface_is_anqpsock(hapd, &from, fromlen)) {
...
int hostapd_ctrl_iface_init(struct hostapd_data *hapd)
{
	...
	if (mkdir(hapd->conf->ctrl_interface, 0777) < 0) {
		if (errno == EEXIST) {
