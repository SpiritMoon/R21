			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	if (addr == NULL) {
		 ...
		 * event here.
		 */
		wpa_printf(MSG_DEBUG, "wam_notif_assoc: Skip event with "
			   "no address");
		return -1;
	}
    ...
    wpa_printf(MSG_INFO,"_GOLSOH_" "STA " MACSTR " - associated",MAC2STR(addr));
...
void hostapd_notif_disassoc(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (addr == NULL) {
		 ...
		 * event here.
		 */
		wpa_printf(MSG_DEBUG, "wam_notif_disassoc: Skip event "
			   "with no address");
		return;
	}
	...
	wpa_printf(MSG_INFO,"_GOLSOH_" "STA " MACSTR " - disassociated",MAC2STR(addr));
