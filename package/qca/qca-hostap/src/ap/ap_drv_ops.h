...
static inline int hostapd_drv_read_sta_info(
	struct hostapd_data *hapd, u8 **buf, const u8 *addr)
{
	if (hapd->driver == NULL || hapd->driver->read_sta_info == NULL)
		return -1;
	return hapd->driver->read_sta_info(hapd->drv_priv, buf, addr);
}

static inline int hostapd_drv_get_sta_idle_time(
	struct hostapd_data *hapd, const u8 *addr, u8 *buf, int len)
{
	if (hapd->driver == NULL || hapd->driver->get_sta_idle_time == NULL)
		return -1;
	return hapd->driver->get_sta_idle_time(addr, buf, len);
}
