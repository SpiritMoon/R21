...
extern Clusterlist_head g_cluster_list;
...
int icm_selector_init(ICM_INFO_T* picm)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"icm_selector_init\n");
    ...
    icm_scan_and_select_channel(ICM_INFO_T* picm, bool setchannel)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"Selecting the best channel...\n");
    print_cluster_list();
    icm_select_home_channel(picm);

    if (setchannel && picm->best_channel > 0) {
        acs_store_best_channel(picm,picm->best_channel);
        drm_log(DRM_LOG_WRITE_TO_SYSLOG,"ACS_CONTROL: Channel select finished, setting & store best channel %d for %s\n",picm->best_channel,picm->radio_ifname);
    ...
    */
    ...
    for (chan_idx = 0; chan_idx < num_chan_to_scan; chan_idx++) {
        ...
        if (ICM_GET_CHANNEL_EXCLUDE(picm, ng_chan[chan_idx]) == TRUE) {
            ...
            printf("%s ng_chan[%d] = %d contiune 001\n",__func__,chan_idx,ng_chan[chan_idx]);
        ...
        if (chanlist->ch[ng_chan[chan_idx] - 1].used_as_secondary_20) {
            ...
            ICM_SET_CHANNEL_USABLITY(picm, ng_chan[chan_idx], 0);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
            printf("%s ng_chan[%d] = %d contiune 002\n",__func__,chan_idx,ng_chan[chan_idx]);
            continue;
        }
        #endif
        ...
        for (overlap_chan_idx = overlap_chan[chan_idx][0]; overlap_chan_idx < overlap_chan[chan_idx][1]+1; overlap_chan_idx++ ) {
            ...
            printf("%s chanlist->ch[%d].num_wnw = %d \n",__func__,overlap_chan_idx-1,chanlist->ch[overlap_chan_idx-1].num_wnw);
            num_ap += chanlist->ch[overlap_chan_idx-1].num_wnw;

            if (chandbginfo != NULL) {
                printf("%s set chan[%d] obss_aps is %d\n",__func__,chan_idx,chandbginfo->num_obss_aps);
		...
		usability /= ((num_ap + 1));
        drm_log(DRM_LOG_WRITE_TO_SYSLOG,"considering the ap of same chanel,chanlist->ch[%d] usability=%d\n",chan_idx,usability);
		...
		if ((ICM_GET_CHANNEL_NOISEFLOOR(picm, ng_chan[chan_idx]) > nf_threshold) ||
			...
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"NOISEFLOOR > nf_threshold interfernece\n");
			ICM_SET_CHANNEL_USABLITY(picm, ng_chan[chan_idx], 0);
			ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
			...
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"there is MWO interfernece, if so degrade the channel by 60 percent,usability=%d\n",usability);
			...
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"there is FHSS interference. Degrade the usability by 40 percent,usability=%d\n",usability);
		...
		if (usability > adjusted_channel_load) {
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"usability > adjusted_channel_load,usability=%d,adjusted load=%d\n",usability,adjusted_channel_load);
			usability = adjusted_channel_load;
		}
		...
		if (ch_bw != ICM_CH_BW_20) {
			printf("ch_bw != ICM_CH_BW_20,usability=%d \n",usability);
		...
		usability *= pow(0.5,icm_get_ap_num_of_the_chan_in_the_cluster(ng_chan[chan_idx],&g_cluster_list));
		drm_log(DRM_LOG_WRITE_TO_SYSLOG,"considering the ap of same chanel in the cluster ,chanlist->ch[%d] usability=%d\n",chan_idx,usability);
	...
	for (chan_idx = 0; chan_idx < num_chan_to_scan; chan_idx++) {
		if (ICM_GET_CHANNEL_USABLITY(picm, ng_chan[chan_idx]) > usability) {
			...
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"select best chan,chanlist->ch[%d] usability=%d\n",chan_idx,usability);
		}
		ICM_DPRINTF(pdev,
				ICM_PRCTRL_FLAG_NONE,
				ICM_DEBUG_LEVEL_DEFAULT,
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"best chan=%d usability=%d\n",best_chan,usability);
...
int icm_select_best_chan_na( ICM_INFO_T* picm, ICM_CH_BW_T ch_bw)
{
    ...
    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ...
        chan_num = ch->channel;
        if(chan_num == 48 || chan_num == 52 || chan_num == 56){
            continue;
        }
        else{
            printf("%s ch %d\n",__func__,chan_num);
        }
		drm_log(DRM_LOG_WRITE_TO_SYSLOG,"chan_num=%d\n",chan_num);
        prichandbginfo = &(decdbginfo[chan_idx].prichaninfo);
        ...
        if ((ch_bw == ICM_CH_BW_80) && !(IEEE80211_IS_CHAN_11AC_VHT80(ch))) {
            ...
            ICM_SET_CHANNEL_USABLITY(picm, chan_num, 0);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"we have Assert protection to ensure that the PHY spec in this case is 11AC\n");
            continue;
        }
        ...
        if (chanlist->ch[chan_idx].used_as_secondary_20) {
            ...
            ICM_SET_CHANNEL_USABLITY(picm, chan_num, 0);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"Check to ensure our proposed primary is not on the secondary 20 MHz of another BSS.\n");
            continue;
        } else {
        ...
        if (chanlist->ch[chan_idx].used_as_secondary_40) {
            ...
            ICM_SET_CHANNEL_USABLITY(picm, chan_num, 0);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
			drm_log(DRM_LOG_WRITE_TO_SYSLOG,"Check to ensure our proposed primary is not on the secondary 40 MHz of another 160/80+80 MHz BSS.\n");
            continue;
        } else {
        ...
        if (ch_bw < ICM_CH_BW_80) {
            ...
            for (bondspan_chan_adj = adj_check_min_adj;
				...
				drm_log(DRM_LOG_WRITE_TO_SYSLOG,"bondspan_chan_num=%d,chan_num=%d\n",bondspan_chan_num,chan_num);
		...
		usability *= pow(0.5,icm_get_ap_num_of_the_chan_in_the_cluster(chan_num,&g_cluster_list));
		drm_log(DRM_LOG_WRITE_TO_SYSLOG,"considering the ap of same chanel in the cluster ,chanlist->ch[%d],chan_num=%d, usability=%d\n",chan_idx,chan_num,usability);
...
int icm_select_home_channel(ICM_INFO_T* picm)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"icm_select_home_channel\n");
    ICM_DEV_INFO_T* pdev = get_pdev();
...
void icm_change_channel(ICM_INFO_T* picm, DCS_INT_TYPE int_type, u_int16_t dcs_enabled)
{
    ...
    if (picm->band == ICM_BAND_5G) {
        ...
        if ((picm->num_usable_ch-1) > picm->cur_ch_idx) {
			...
			drm_wifi_down(picm->radio_ifname); //pengdecai for down up wifi when channnel change
            system(cmd);
			drm_wifi_up(picm->radio_ifname); //pengdecai for down up wifi when channnel change
        } else {
            /* We do not have any good channels, so re-scan */
            //icm_scan_and_select_channel(picm, TRUE);
            /*ACS control*/
            drm_log(DRM_LOG_NORMAL,"ACS_CONTROL: recv acs request from driver, start acs.\n");
            picm->acs_force = 1;
        }
    } else {
        //icm_scan_and_select_channel(picm, TRUE);
                                  ...
                                  int arraysize)
{
    ...
    for (chan_idx = 0; chan_idx < arraysize; chan_idx++) {
        ...
        ICM_SELDBG_PRINTENTRY_STRCONV_CONS(pdev,
                                           ...
                                           decdbginfo[chan_idx].is_bestchan,
                                           icm_bool_shortstr);
        drm_log(DRM_LOG_WRITE_TO_SYSLOG,"ACS_CONTROL: Select detail Chan(%d) NumOBSS(%d) MsrdUsab(%d) Usab(%d) isBest(%d)\n",prichandbginfo->chan_num,prichandbginfo->num_obss_aps,decdbginfo[chan_idx].measured_usability,decdbginfo[chan_idx].usability,decdbginfo[chan_idx].is_bestchan);
