...
struct sta_info * ap_get_sta_by_ip(struct hostapd_data *hapd, const be32 staip)
{
	struct sta_info *sta;
...
void ap_free_sta(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	send_user_sync_info(hapd, sta, SYNC_STA_DEL);
...
void ap_free_sta_no_notice(struct hostapd_data *hapd, struct sta_info *sta)
{
	int set_beacon = 0;
...
#if 0
struct sta_sync *sync_sta_get(u8 *addr)
{
    int i;
	struct sta_sync *tmp = NULL;
	for(i = 0;i < 16;i++){
        if(CHECK_MAC(vap[i].mac) != 0){
           tmp = vap[i].sta_list;
		   while(tmp != NULL){
                if(os_memcmp(addr, tmp->addr, ETH_ALEN) ==0)
					return tmp;
				tmp = tmp->next;
		   }        
		}
...
#endif
void ap_sta_roaming(struct hostapd_data *hapd, struct sta_info *sta)
{

	struct sta_sync *old_sta;
	
    //old_sta = sync_sta_get(sta->addr);
	old_sta = sta_sync_hash_get(sta->addr);
	if(old_sta == NULL)
		return;
	sta->ipaddr = old_sta->ipaddr;
	sta->roam_flag = 1;
   // send_user_roaming_info(hapd,old_sta);
	wpa_printf(MSG_DEBUG,"<=====ap_sta_roaming====>");
}
			   ...
			   int authorized)
{
	...
	if (authorized) {
		...
		wpa_printf(MSG_INFO,"_GOLSOH_" AP_STA_CONNECTED MACSTR, MAC2STR(sta->addr));
		...
		if (hapd->msg_ctx_parent &&
			...
			wpa_msg(hapd->msg_ctx_parent, MSG_INFO,
				...
				MAC2STR(sta->addr));
		wpa_printf(MSG_INFO,"_GOLSOH_" AP_STA_DISCONNECTED MACSTR, MAC2STR(sta->addr));
		sta->flags &= ~WLAN_STA_AUTHORIZED;
	}
