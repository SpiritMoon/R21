...
extern LBD_STATUS wlanifBSteerControlDisable(wlanifBSteerControlHandle_t handle);
...
static void wlanifListenInitCB(void) {
    if (wlanifBSteerEventsEnable(wlanifState.bsteerEventsHandle) != LBD_OK) {
        ...
        // failure code.
      printf( "%s: wlanifBSteerEventsEnable error exit!\n",__func__);
	// wlanifBSteerControlDisable(wlanifState.bsteerEventsHandle);
        exit(1);
    }
}
...
/*Begin:pengdecai for dcm*/
void
han_dcm_signal_handle_get_rssithreshold(void)
{
printf("%s \n",__func__);
      han_dcm_get_all_rssithreshold(wlanifState.bsteerControlHandle);
}
/*End:pengdecai for dcm*/
...
LBD_STATUS wlanif_han_dcm_balance_ioctl(
                  char * ifname,
	            ...
	            unsigned char denycnt)
{
	...
	if (!wlanifState.bsteerControlHandle) {
		printf("wlanifState.bsteerControlHandle = NULL\n");
		return LBD_NOK;
	}
//	printf("lbd_3\n");

	return wlanif_han_dcm_balance_ioctl_done(
		wlanifState.bsteerControlHandle,ifname,option,
		mac,channel,denycnt);
}
