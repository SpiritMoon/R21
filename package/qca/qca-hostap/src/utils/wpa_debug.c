...
static int wpa_debug_outfile = 1;
...
void wpa_debug_print_timestamp(void)
{
	...
	if (out_file) {
		...
		fprintf(out_file, "[PID:%u %d-%02d-%02d %02d:%02d:%02d]: ",wam_pid,(1900+time->tm_year),
		    (1+time->tm_mon),time->tm_mday,time->tm_hour,time->tm_min,time->tm_sec);
	} else
#endif /* CONFIG_DEBUG_FILE */
	//printf("%ld.%06u: ", (long) tv.sec, (unsigned int) tv.usec);
	printf("[PID:%u]%d-%02d-%02d %02d:%02d:%02d: ",wam_pid,(1900+time->tm_year),(1+time->tm_mon),
...
void wam_syslog_err(char *format,...)
...
void wam_syslog_warning(char *format,...)
...
void wam_syslog_info(char *format,...)
...
void wam_syslog_debug(char *format,...)
	...
	if (level >= wpa_debug_level) {
		...
		if (wpa_debug_syslog) {
			...
			*/
			...
			os_memcpy(sbuf,iface_name,strlen(iface_name));
			os_memcpy(sbuf+strlen(iface_name),":",1);
			os_memcpy(sbuf+strlen(iface_name)+1,buf,sizeof(buf));
			switch(level) {
		        case MSG_ERROR:
			        wam_syslog_err(sbuf);
			        break;
		        case MSG_WARNING:
			        wam_syslog_warning(sbuf);
			        break;
		        case MSG_INFO:
			        wam_syslog_info(sbuf);
			        break;
		        case MSG_DEBUG:
		        case MSG_MSGDUMP:
			        wam_syslog_debug(sbuf);
			    break;
		        default:
			    break;
	       }		
 ...
 * Note: New line '\n' is added to the end of the text when printing to stdout.
 */
void wpa_printf_new(int level, const char *fmt, ...)
{
	va_list ap;
		    ...
		    const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_printf(MSG_ERROR, "wam_logger: Failed to allocate "
			   "message buffer");
		return;
	}
	va_start(ap, fmt);
	...
	if (hostapd_logger_cb)
		...
		wpa_printf(MSG_DEBUG, "wam_logger: STA " MACSTR " - %s",
			   MAC2STR(addr), buf);
	else
		wpa_printf(MSG_DEBUG, "wam_logger: %s", buf);
	os_free(buf);
}
