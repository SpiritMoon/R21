...
find_ni_node_by_mac(struct MC_LIST_UPDATE* list_entry)
{
	...
	if(nt){
		ni = ieee80211_find_node(nt,list_entry->grp_member);
	}
...
set_igmp_other_entry(struct MC_LIST_UPDATE* list_entry)
{
	 ...
	 if(NULL == find_ni_node_by_mac(list_entry)){
...
find_group_member_by_mac(struct ieee80211_node *ni)
{
	...
	if(vap->iv_me->me_debug == 3){
		printk("igmp driver:the sta  %02X:%02X:%02X:%02X:%02X:%02X is not a member of the vap mc group",
...
void send_igmp_snooping_sta_leave(struct ieee80211_node *ni)
{
	...
	if(ni->ni_vap->iv_me->me_debug == 3){
		printk("Leave Sta MAC: %02x:%02x:%02x:%02x:%02x:%02x \n",\
			ni->ni_macaddr[0],ni->ni_macaddr[1],ni->ni_macaddr[2],\
			ni->ni_macaddr[3],ni->ni_macaddr[4],ni->ni_macaddr[5]);
	}
...
han_ieee80211_me_SnoopListDump(struct ieee80211vap* vap)
{
        ...
        snp_list != NULL)
    {
        ...
        TAILQ_FOREACH(grp_list,&snp_list->msl_node,grp_list){
            printk("group addr %02X:%02X:%02X:%02X:%02X:%02X \n",grp_list->group_addr[0],
            ...
            TAILQ_FOREACH(grp_member_list, &grp_list->src_list,member_list){
				...
				/* modefied by pengdecai for igmpsnp*/
				printk("member addr: \n");
...
ieee80211_han_ioctl_igmp_snooping(struct net_device *dev,struct han_ioctl_priv_args *a,struct iwreq *iwr)
{
	...
	switch (a->u.igmp.subtype) {
		...
		case HAN_IOCTL_IGMPSNP_STATUS:
			printk("status:\n");
			printk("snoop_enable = %d\n",vap->iv_me->mc_snoop_enable);
			printk("mcast_enable = %d\n",vap->iv_me->mc_mcast_enable);
