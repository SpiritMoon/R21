...
{
    struct ath_softc_net80211 *scn;
#if ATOPT_BACKGROUND_SCANNING
	//zhaoyang modifies for background scanning 2016-03-23
    ...
    if (wbuf_get_pktlen(wbuf) < IEEE80211_ACK_LEN) {
		#if ATOPT_BACKGROUND_SCANNING
		if (!ic->ic_is_mode_offload(ic)) //zhaoyang modifies for background scanning 2016-03-23
		#endif
    	...
    	if (!ic->ic_is_mode_offload(ic)) {
    		...
    		ieee80211_input_monitor(ic, wbuf, rx_status);
#else	
        struct ieee80211_rx_status rs;
        ATH_RXSTAT2IEEE(ic, rx_status, &rs);
        ieee80211_input_monitor(ic, wbuf, &rs);
...
#if ATOPT_RESTRAIN_QUEUE
#ifndef SND_TIME_PER_PACKET
#define SND_TIME_PER_PACKET 4
#endif
static OS_TIMER_FUNC(ath_restarin_queue_2g_func)
{
	...
	flush_pkt_num = ic->ic_cur_min_dwell * 40 / 50 / (SND_TIME_PER_PACKET + 1);

	do {
		skb = dequeue_restrain_q(ic->ic_curchan->ic_ieee);
		if (skb) {
			for (i=0; i<SND_TIME_PER_PACKET; i++) {
				skb1 = skb_copy(skb, GFP_ATOMIC);
				if (skb1) {
					ieee80211_send_mgmt(ic->ic_scan_vap, ic->ic_scan_vap->iv_bss, skb1, true);
				}
			}
			ieee80211_send_mgmt(ic->ic_scan_vap, ic->ic_scan_vap->iv_bss, skb, true);
			flush_pkt_num -= (SND_TIME_PER_PACKET + 1);
		}
	} while(skb && (flush_pkt_num >= (SND_TIME_PER_PACKET + 1)));
...
#if ATOPT_RESTRAIN_QUEUE
	printk("init 2g restrain func timer\n");
	OS_INIT_TIMER(ic->ic_osdev, &(ic->ic_restrain_2g_timer), 
		ath_restarin_queue_2g_func, (void *) (ic));
	init_restrain_q(true);
#endif
...
ath_detach(struct ath_softc_net80211 *scn)
{
	...
	/*End:pengdecai for han netlink*/	
#if ATOPT_RESTRAIN_QUEUE
	OS_FREE_TIMER(&ic->ic_restrain_2g_timer);
	destroy_restrain_q(true);
#endif
