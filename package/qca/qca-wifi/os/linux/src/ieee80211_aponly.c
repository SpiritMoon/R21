...
#if ATOPT_DCM
        if (rs->rs_rssi < vap->iv_lowrssi_threshold) 
        {
            if(type == IEEE80211_FC0_TYPE_MGT && \
...
/*
* zhaoyang add for background scanning 2016-03-18
*/

int 
ath_net80211_copy2monitor(struct ieee80211com *ic, wbuf_t wbuf, 
...
#if ATOPT_WIRELESS_QOS
static INLINE int
ieee80211_dscp_default_tid_aponly(struct ieee80211vap *vap, wbuf_t wbuf)
{
    u_int32_t is_igmp;
	u_int8_t tos;
	u_int8_t dscp;
...
ieee80211_dscp_override(struct ieee80211vap *vap, wbuf_t wbuf)
{
    int tid = -1;
#if ATOPT_WIRELESS_QOS
	tid = ieee80211_dscp_default_tid_aponly(vap,wbuf);
#endif
	...
	*/
	...
	if(vap->priv_wmm.debug && ieee80211_vap_wme_is_set(vap)){
		if(vap->priv_wmm.debug == 1){
				...
				printk("DSCP->WMM:set(4/4): End:v_wme_ac=%d ,v_pri =%d\n",v_wme_ac,v_pri);
