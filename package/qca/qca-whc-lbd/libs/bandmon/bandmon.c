...
static void bandmonReadConfig(void) {
	...
	g_own_ap_state.maxNumDiff =
	 profileGetOptsInt(mdModuleID_BandMon,
                          BANDMON_MU_MAX_STA_NUM_DIFF,
                          bandmonElementDefaultTable);
    ...
    bandmonState.config.probeCountThreshold =
        profileGetOptsInt(mdModuleID_BandMon,
                          ...
                          bandmonElementDefaultTable);
	g_own_ap_state.rssiThreshold = bandmonState.config.rssiSafetyThreshold;
}
...
static void bandmonHandleVAPRestartEvent(struct mdEventNode *event) {
    ...
    if (bandmonInitializeChannels() != LBD_OK) {
        dbgf(bandmonState.dbgModule, DBGERR,
             ...
             __func__);
              printf( "%s: Failed to fetch active channels; aborting\n",__func__);
              exit(1);
    }
...
struct han_rssi_info{
	unsigned char vaprssi[16];
	unsigned int vapcnt;
};


static LBD_BOOL han_dcm_get_rssi_callback(stadbEntry_handle_t entryHandle, 
	                             stadbEntry_bssStatsHandle_t bssHandle,
                                 void *cookie)
{
  //  unsigned char  * rssi_aver = (unsigned char *) cookie;
    struct han_rssi_info *prssi = (struct han_rssi_info *)cookie;
	...
	if(rssi){
		prssi->vaprssi[prssi->vapcnt] = rssi;
		prssi->vapcnt ++;
	}
...
LBD_BOOL  han_dcm_check_is_overload(unsigned char utilization,unsigned char channel)
{
...
unsigned char
han_dcm_get_rssi(stadbEntry_handle_t entry)
{
   int i;
   unsigned long rssi_total = 0;
   unsigned char rssi_aver = 0;
   struct han_rssi_info rssi;
   
   memset(&rssi,0x0,sizeof(struct han_rssi_info));
   
   if (stadbEntry_iterateBSSStats(entry,
								  han_dcm_get_rssi_callback,
								  &rssi, NULL, NULL) != LBD_OK) {
	//   const struct ether_addr *addr = stadbEntry_getAddr(entry);
   }

   for(i = 0;i < rssi.vapcnt;i ++){
   	   rssi_total += rssi.vaprssi[i];
   }

   if(rssi.vapcnt){
  	 rssi_aver = rssi_total/rssi.vapcnt;
   }
   return rssi_aver;
}

//pengdecai added for dcm
void han_handle_dcm_clear_blance_list(stadbEntry_handle_t entry, void *cookie)
{
	unsigned int timedif = stadbEntry_getDcmDiff(entry);
	unsigned char channel = *((unsigned char *)(cookie));
	const struct ether_addr *staAddr = stadbEntry_getAddr(entry);
	if(stadbEntry_getDcmFlag(entry)){
		if(timedif < 1200){/*20 minite*/
			return ;
		}
		dcm_log(SYSLOG_BALANCE,"[7/8]STA %s has not upgrade in 20 minutes delelte it from blacklist\n",ether_sprintf(staAddr->ether_addr_octet));
		if(wlanif_han_dcm_balance_ioctl(NULL,HAN_IOCTL_DCM_BLANCE_DELMAC,\
		              staAddr->ether_addr_octet,\
		              channel,0)){
		         dcm_log(SYSLOG_BALANCE,"[8/8]STA %s ioctl balance delete from blacklist error\n",ether_sprintf(staAddr->ether_addr_octet));
	      }else{
			stadbEntry_clearDcmFlag(entry);
			dcm_log(SYSLOG_BALANCE,"[8/8]STA %s ioctl balance delete from blacklist success\n",ether_sprintf(staAddr->ether_addr_octet));
		}
	}
	...
	if(stadbEntry_isSetwifiname(entry)){
		...
		dcm_log(SYSLOG_5GFIRST,"[7/8]STA %s has not upgrade in 20 minutes delelte it from blacklist\n",ether_sprintf(staAddr->ether_addr_octet));

		char *ifname = stadbEntry_getwifiname(entry);
		
		if(wlanif_han_dcm_balance_ioctl(ifname,HAN_IOCTL_DCM_LBD_DELMAC,\
			              staAddr->ether_addr_octet,\
			              channel,0)){
		         dcm_log(SYSLOG_5GFIRST,"[8/8]STA %s ioctl 5G first delete from %s blacklist error\n",ether_sprintf(staAddr->ether_addr_octet),ifname);
		}else{
			   stadbEntry_clearwifiname(entry);
			   dcm_log(SYSLOG_5GFIRST,"[8/8]STA %s ioctl 5G first delete from %s blacklist success\n",ether_sprintf(staAddr->ether_addr_octet),ifname);
...
#if 0
//pengdecai added for dcm
void han_handle_dcm_util(stadbEntry_handle_t entry, void *cookie)
{
	unsigned char channel = *((unsigned char *)(cookie));
	int dney_cnt = 0;
	int isDualBand = 0;
    ...
    printf("%s: util  MAC " lbMACAddFmt(":")"\n",
             __func__, lbMACAddData(staAddr->ether_addr_octet));
    unsigned char changecnt = 0;
	changecnt = stadbEntry_getBandsChangeCnt(entry);
	...
	timedif = stadbEntry_getDcmDif(entry);
	
	//printf("lbd_0\n");

//	printf("lbd_0 cap_cnt = %d,isdualBand = %d,rssi = %d,dney_cnt = %d,timedif = %d\n",
//		   changecnt,isDualBand,rssi,dney_cnt,timedif);
...
#endif
//pengdecai added for dcm
static void  han_handle_dcm_do(unsigned char channel)
{
//	return ;
   //u_int8_t i;
   #if 0
   ...
   if (stadb_iterate(han_handle_dcm_clear_blance_list, (void *) &channel) != LBD_OK) {
	   dbgf(bandmonState.dbgModule, DBGERR,
