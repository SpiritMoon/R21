...
#else
    if (!ieee80211_vap_active_is_set(vap) || ieee80211_vap_scanning_is_set(vap) ||
        wlan_scan_in_progress_ic(vap->iv_ic)) 
#endif
        return true;
    ...
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {                               
        if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
            if  (ieee80211_vap_can_pause(vap) == false) {
				...
				/*There maybe multi-vap on this ic, the posterior vap may not satisfy pause
				condition while the front vap may be paused. So we need unpause the paused vap*/
