		     ...
		     int sock, int sock6, int auth)
{
    ...
    if(s < 0){
        wpa_printf(MSG_ERROR,"create ioctl socket failed");
		return -1;
	}
	strlcpy(ifr.ifr_name,"br-wan",sizeof(ifr.ifr_name));
	if(ioctl(s,SIOCGIFADDR,&ifr) != 0){
        wpa_printf(MSG_ERROR,"ioctl to get bridge ipaddr failed");
		close(s);
		return -1;
	}
	br_addr = (struct sockaddr_in *)&ifr.ifr_addr;
	if (conf->force_client_addr) {
        ...
        os_memset(&claddr, 0, sizeof(claddr));
		claddr.sin_family = AF_INET;
		claddr.sin_addr.s_addr = br_addr->sin_addr.s_addr;
		claddr.sin_port = htons(0);
		cl_addr = (struct sockaddr *) &claddr;
		claddrlen = sizeof(claddr);
		if (bind(sel_sock, cl_addr, claddrlen) < 0) {
			perror("bind[radius]");
			wpa_printf(MSG_ERROR,"bind radius socket to bridge failed");
			close(s);
			return -1;
		}
		wpa_printf(MSG_DEBUG,"++++>bind radius socket to bridge success<++++");
	}
