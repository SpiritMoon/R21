...
#if 0
static int
atheros_read_sta_driver_info(void *priv, struct hostap_sta_driver_info *info,
			     const u8 *addr)
{
	struct atheros_driver_data *drv = priv;
	struct ieee80211req_sta_req stats;
	...
	memset(&stats, 0, sizeof(stats));
	memcpy(stats.is_u.macaddr, addr, IEEE80211_ADDR_LEN);
	if (set80211priv(drv, IEEE80211_IOCTL_STA_INFO,
			 &stats, sizeof(stats))) {
		wpa_printf(MSG_DEBUG, "%s: Failed to fetch STA info (addr "
			   MACSTR ")", __func__, MAC2STR(addr));
		printf("Failed to get station stats information element.\n");
		return -1;
	}
    wpa_printf(MSG_DEBUG, "%s: success to fetch STA info (addr "
			   MACSTR ")", __func__, MAC2STR(stats.info[0].isi_macaddr));
	info->isi_inact= stats.info[0].isi_inact;
...
static int
atheros_read_sta_driver_info(void *priv, u8 **buf,
			     const u8 *addr)
{
	struct atheros_driver_data *drv = priv;
    struct iwreq iwr;
	...
	*buf = malloc(48*1024);
	if(*buf == NULL){
		wpa_printf(MSG_DEBUG,"read_sta_driver_info malloc failed");
        return -1;
	}
    iwr.u.data.pointer = *buf;
	iwr.u.data.length = 48*1024;
	iwr.u.data.flags = 0;
	
    len = ioctl(drv->ioctl_sock, IEEE80211_IOCTL_STA_INFO, &iwr);
	if(len < 0){
		wpa_printf(MSG_DEBUG,"IEEE80211_IOCTL_STA_INFO failed!:%d (%s)",
...
static int
atheros_get_sta_idle_time(const u8 *addr, u8 *buf, int len)
{
    struct ieee80211req_sta_info *si;
    u8 *pos;
	...
	while(len >= sizeof(struct ieee80211req_sta_info)){
		...
		if(os_memcmp(addr, si->isi_macaddr, 6) == 0){
            return si->isi_inact;
		}
		pos += si->isi_len;
		len -= si->isi_len;
		 ...
		 u16 status_code, const u8 *ie, size_t len)
{
	...
	wpa_printf(MSG_INFO,"_GOLSOH_" "STA " MACSTR " - authticated",MAC2STR(addr));
	return ret;
}
...
atheros_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	...
	if (linux_br_get(brname, drv->iface) == 0) {
		drv->br_ifindex = if_nametoindex(brname);
		wpa_printf(MSG_DEBUG, "Interface %s in bridge %s(%d)", 
			drv->iface, brname, drv->br_ifindex);
	}
	...
	if (linux_br_get(brname, drv->iface) < 0) {
		perror("bridge name: fail to get");
		wpa_printf(MSG_ERROR, "bridge name: fail to get");
		goto bad;
	}
...
*/
	...
	rtnl_neigh_set_ifindex(rn, drv->br_ifindex);
