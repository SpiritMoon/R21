...
HANBool Parse_ACS(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	switch(parse_packet->u.acs.msgtype)
	{
		...
		case Sequence_resp:
		{
			...
			if(parse_packet->u.acs.head.protocolver > 0)
			{
...
unsigned short Get_Hccp_Elem_Type(char **buf)
{
...
unsigned short Get_Hccp_Elem_Len(char **buf)
{
...
unsigned char Tlv_Parse_8_Bit(char **buf, unsigned short len)
{
...
unsigned short Tlv_Parse_16_Bit(char **buf,  unsigned short len)
{
...
unsigned int Tlv_Parse_32_Bit(char **buf,  unsigned short len)
{
...
void Tlv_Parse_String(char **buf, char *str, unsigned short len)
{
	memcpy(str, *buf, len);
...
void Tlv_Parse_MAC(char **buf, char *str,  unsigned short len)
{
...
void Unknown_Type_Handle(char **buf, unsigned short len)
{
	*buf += len;
}
...
HANBool Parse_DBCP_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		hccp_elem_type = Get_Hccp_Elem_Type(&buf);
		hccp_elem_len = Get_Hccp_Elem_Len(&buf);
		...
		switch(hccp_elem_type)
		{
			...
			{
				parse_packet->u.dbcp.priority = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}				
			case DBCP_PKT_CONFIG_SEQUENCE:
			{
				parse_packet->u.dbcp.config_sequence = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}
			case DBCP_PKT_PRODUCT_TYPE:
			{
				parse_packet->u.dbcp.product_type = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}
			case DBCP_PKT_MAC:
			{
				Tlv_Parse_MAC(&buf, parse_packet->u.dbcp.mac,  hccp_elem_len);
				break;
			}				
			case DBCP_PKT_STATE:
			{
				parse_packet->u.dbcp.state = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}				
			case DBCP_PKT_RADIOCNT:
			{
				parse_packet->u.dbcp.radiocnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				...
				for(i = 0; i < parse_packet->u.dbcp.radiocnt && i < L_RADIO_NUM; i++)
				{
					while(1)
					{
						...
						{
							Tlv_Parse_8_Bit(&buf, hccp_elem_len);
							break;
						}
						...
						switch(hccp_elem_type)
						{
							...
							{
								parse_packet->u.dbcp.WTP_Radio[i].radioid = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case DBCP_PKT_WTP_RADIO_CHANNEL:
							{
								parse_packet->u.dbcp.WTP_Radio[i].channel = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case DBCP_PKT_WTP_RADIO_TXPOWER:
							{
								parse_packet->u.dbcp.WTP_Radio[i].txpower = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case DBCP_PKT_WTP_RADIO_RSSI:
							{
								parse_packet->u.dbcp.WTP_Radio[i].rssi = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							default:
								Unknown_Type_Handle(&buf, hccp_elem_len);
								break;	
						}
					}
				}
...
HANBool Parse_RSCP_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			...
			{
				parse_packet->u.rscp.priority = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}				
			case RSCP_PKT_CONFIG_SEQUENCE:
			{
				parse_packet->u.rscp.config_sequence = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}
			case RSCP_PKT_PRODUCT_TYPE:
			{
				parse_packet->u.rscp.product_type = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}
			case RSCP_PKT_MAC:
			{
				Tlv_Parse_MAC(&buf, parse_packet->u.rscp.mac,  hccp_elem_len);
				break;
			}				
			case RSCP_PKT_COUNT:
			{
				parse_packet->u.rscp.count = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				...
				for(i = 0; i < parse_packet->u.rscp.count && i < MAX_CLUSTER_AP; i++)
				{
					while(1)
					{
						...
						switch(hccp_elem_type)
						{
							...
							{
								Tlv_Parse_MAC(&buf, parse_packet->u.rscp.cluster_member[i].mac,  hccp_elem_len);
								break;
							}								
							case RSCP_PKT_CLUSTER_MEMBER_ROLE:
							{
								parse_packet->u.rscp.cluster_member[i].role = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}								
							case RSCP_PKT_CLUSTER_MEMBER_STATE:
							{
								parse_packet->u.rscp.cluster_member[i].state = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}								
							case RSCP_PKT_CLUSTER_MEMBER_IP:
							{
								parse_packet->u.rscp.cluster_member[i].ip = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
								break;
							}								
							case RSCP_PKT_CLUSTER_MEMBER_RADIOCNT:
							{
								parse_packet->u.rscp.cluster_member[i].radiocnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								...
								for(j = 0; j < parse_packet->u.rscp.cluster_member[i].radiocnt && j < L_RADIO_NUM; j++)
								{
									while(1)
									{
										...
										switch(hccp_elem_type)
										{
											...
											{
												parse_packet->u.rscp.cluster_member[i].WTP_Radio[j].radioid = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RSCP_PKT_CLUSTER_MEMBER_WTP_RADIO_CHANNEL:
											{
												parse_packet->u.rscp.cluster_member[i].WTP_Radio[j].channel = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RSCP_PKT_CLUSTER_MEMBER_WTP_RADIO_TXPOWER:
											{
												parse_packet->u.rscp.cluster_member[i].WTP_Radio[j].txpower = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RSCP_PKT_CLUSTER_MEMBER_WTP_RADIO_RSSI:
											{
												parse_packet->u.rscp.cluster_member[i].WTP_Radio[j].rssi = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											default:
												Unknown_Type_Handle(&buf, hccp_elem_len);
...
HANBool Parse_RSRP_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			...
			{
				parse_packet->u.rsrp.priority = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}
			case RSRP_PKT_CONFIG_SEQUENCE:
			{
				parse_packet->u.rsrp.config_sequence = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}	
			case RSRP_PKT_PRODUCT_TYPE:
			{
				parse_packet->u.rsrp.product_type = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}	
			case RSRP_PKT_MAC:
			{
				Tlv_Parse_MAC(&buf, parse_packet->u.rsrp.mac,  hccp_elem_len);
				break;
			}	
			case RSRP_PKT_STATE:
			{
				parse_packet->u.rsrp.state = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}
			case RSRP_PKT_IP:
			{
				parse_packet->u.rsrp.ip = Tlv_Parse_32_Bit(&buf,  hccp_elem_len);
				break;
			}
			case RSRP_PKT_AP_VERSION:
			{
				Tlv_Parse_String(&buf, parse_packet->u.rsrp.ap_version, hccp_elem_len);
				break;
			}
			case RSRP_PKT_AP_NAME:
			{
				Tlv_Parse_String(&buf, parse_packet->u.rsrp.ap_name, hccp_elem_len);
				break;
			}
			case RSRP_PKT_RADIOCNT:
			{
				parse_packet->u.rsrp.radiocnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				for(i = 0; i < parse_packet->u.rsrp.radiocnt && i < L_RADIO_NUM; i++)
				{
					while(1)
					{
						...
						switch(hccp_elem_type)
						{
							...
							{
								parse_packet->u.rsrp.WTP_Radio[i].radioid = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case RSRP_PKT_WTP_RADIO_CHANNEL:
							{
								parse_packet->u.rsrp.WTP_Radio[i].channel = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case RSRP_PKT_WTP_RADIO_TXPOWER:
							{
								parse_packet->u.rsrp.WTP_Radio[i].txpower = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case RSRP_PKT_WTP_RADIO_RSSI:
							{
								parse_packet->u.rsrp.WTP_Radio[i].rssi = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							default:
								Unknown_Type_Handle(&buf, hccp_elem_len);
...
HANBool Parse_RIRP_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			case RIRP_PKT_MAC:
			{
				Tlv_Parse_MAC(&buf, parse_packet->u.rirp.mac,  hccp_elem_len);
				break;
			}				
			case RIRP_PKT_NEIGHBOR_COUNT:
			{
				parse_packet->u.rirp.neighbor_count = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				...
				for(i = 0; i < parse_packet->u.rirp.neighbor_count && i < MAX_CLUSTER_AP; i++)
				{
					while(1)
					{
						...
						switch(hccp_elem_type)
						{
							...
							{
								Tlv_Parse_MAC(&buf, parse_packet->u.rirp.cluster_neighbor[i].mac,  hccp_elem_len);
								break;
							}
							case RIRP_PKT_CLUSTER_NEIGHBOR_RADIOCNT:
							{
								parse_packet->u.rirp.cluster_neighbor[i].radiocnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								...
								for(j = 0; j < parse_packet->u.rirp.cluster_neighbor[i].radiocnt && j < L_RADIO_NUM; j++)
								{
									while(1)
									{
										...
										switch(hccp_elem_type)
										{
											...
											{
												parse_packet->u.rirp.cluster_neighbor[i].WTP_Radio[j].radioid = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RIRP_PKT_CLUSTER_NEIGHBOR_WTP_RADIO_CHANNEL:
											{
												parse_packet->u.rirp.cluster_neighbor[i].WTP_Radio[j].channel = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RIRP_PKT_CLUSTER_NEIGHBOR_WTP_RADIO_TXPOWER:
											{
												parse_packet->u.rirp.cluster_neighbor[i].WTP_Radio[j].txpower = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RIRP_PKT_CLUSTER_NEIGHBOR_WTP_RADIO_RSSI:
											{
												parse_packet->u.rirp.cluster_neighbor[i].WTP_Radio[j].rssi = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											default:
												Unknown_Type_Handle(&buf, hccp_elem_len);
...
HANBool Parse_RICP_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			case RICP_PKT_OP:
			{
				parse_packet->u.ricp.op = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				break;
			}				
			case RICP_PKT_VIP:
			{
				parse_packet->u.ricp.vip = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
				break;
			}
			case RICP_PKT_VIP_NETMASK:
			{
				parse_packet->u.ricp.vip_netmask = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
				break;
			}
			case RICP_PKT_COUNT:
			{
				parse_packet->u.ricp.count = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				...
				for(i = 0; i < parse_packet->u.ricp.count && i < MAX_CLUSTER_AP; i++)
				{
					while(1)
					{
						...
						switch(hccp_elem_type)
						{
							case RICP_PKT_MAC:
							{
								Tlv_Parse_MAC(&buf, parse_packet->u.ricp.mac[i],  hccp_elem_len);
								break;
							}							
							default:
								Unknown_Type_Handle(&buf, hccp_elem_len);
...
HANBool Parse_RISP_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			...
			{
				parse_packet->u.risp.ACS_sequence = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
				break;
			}				
			case RISP_PKT_MEM_NUM:
			{
				parse_packet->u.risp.Mem_num = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
				...
				for(i = 0; i < parse_packet->u.risp.Mem_num && i < MAX_CLUSTER_AP; i++)
				{
					while(1)
					{
						...
						switch(hccp_elem_type)
						{
							...
							{
								parse_packet->u.risp.WTP_RF[i].role = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case RISP_PKT_WTP_RF_AP_BASE_MAC:
							{
								Tlv_Parse_MAC(&buf, parse_packet->u.risp.WTP_RF[i].ap_base_mac,  hccp_elem_len);
								break;
							}
							case RISP_PKT_WTP_RF_IPADDR:
							{
								parse_packet->u.risp.WTP_RF[i].ipaddr = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
								break;
							}
							case RISP_PKT_WTP_RF_PRIORITY:
							{
								parse_packet->u.risp.WTP_RF[i].priority = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
								break;
							}
							case RISP_PKT_WTP_RF_RADIOCNT:
							{
								parse_packet->u.risp.WTP_RF[i].radiocnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								...
								for(l = 0; l < parse_packet->u.risp.WTP_RF[i].radiocnt && l < L_RADIO_NUM; l++)
								{
									while(1)
									{
										...
										switch(hccp_elem_type)
										{
											...
											{
												parse_packet->u.risp.WTP_RF[i].WTP_Radio[l].radioid = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RISP_PKT_WTP_RF_WTP_RADIO_CHANNEL:
											{
												parse_packet->u.risp.WTP_RF[i].WTP_Radio[l].channel = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RISP_PKT_WTP_RF_WTP_RADIO_TXPOWER:
											{
												parse_packet->u.risp.WTP_RF[i].WTP_Radio[l].txpower = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											case RISP_PKT_WTP_RF_WTP_RADIO_RSSI:
											{
												parse_packet->u.risp.WTP_RF[i].WTP_Radio[l].rssi = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												break;
											}
											default:
												Unknown_Type_Handle(&buf, hccp_elem_len);
							...
							{
								parse_packet->u.risp.WTP_RF[i].neighbor_cnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								...
								for(j = 0; j < parse_packet->u.risp.WTP_RF[i].neighbor_cnt && j < MAX_CLUSTER_AP; j++)
								{
									while(1)
									{
										...
										switch(hccp_elem_type)
										{
											...
											{
												Tlv_Parse_MAC(&buf, parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].ap_base_mac,  hccp_elem_len);
												break;
											}
											case RISP_PKT_WTP_RF_RSSI_OF_OTHERS_RADIOCNT:
											{
												parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].radiocnt = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
												...
												for(k = 0; k < parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].radiocnt && k < L_RADIO_NUM; k++)
												{
													while(1)
													{
														...
														switch(hccp_elem_type)
														{
															...
															{
																parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].ap_radio[k].radioid = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
																break;
															}
															case RISP_PKT_WTP_RF_RSSI_OF_OTHERS_AP_RADIO_CHANNEL:
															{
																parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].ap_radio[k].channel = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
																break;
															}
															case RISP_PKT_WTP_RF_RSSI_OF_OTHERS_AP_RADIO_TXPOWER:
															{
																parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].ap_radio[k].txpower = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
																break;
															}
															case RISP_PKT_WTP_RF_RSSI_OF_OTHERS_AP_RADIO_RSSI:
															{
																parse_packet->u.risp.WTP_RF[i].rssi_of_others[j].ap_radio[k].rssi = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
																break;
															}
															default:
																Unknown_Type_Handle(&buf, hccp_elem_len);
...
HANBool Parse_DCM_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			...
			{
				parse_packet->u.dcm.radionum = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
				...
				for(i = 0; i < parse_packet->u.dcm.radionum && i < L_RADIO_NUM; i++)
				{
					while(1)
					{
						...
						switch(hccp_elem_type)
						{
							...
							{
								parse_packet->u.dcm.radio[i].util = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case DCM_PKT_RADIO_STANUM:
							{
								parse_packet->u.dcm.radio[i].stanum = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}
							case DCM_PKT_RADIO_BANDTYPE:
							{
								parse_packet->u.dcm.radio[i].bandtype = Tlv_Parse_8_Bit(&buf, hccp_elem_len);
								break;
							}							
							default:
								Unknown_Type_Handle(&buf, hccp_elem_len);
...
HANBool Parse_ACS_Tlv(char *buf, Hccp_Protocol_Struct *parse_packet)
{
	...
	while(1)
	{
		...
		switch(hccp_elem_type)
		{
			...
			{
				parse_packet->u.acs.msgtype = Tlv_Parse_16_Bit(&buf, hccp_elem_len);
				break;
			}				
			case ACS_PKT_SEQ_NUM:
			{
				parse_packet->u.acs.seq_num = Tlv_Parse_32_Bit(&buf, hccp_elem_len);
				break;
			}
			case ACS_PKT_MAC:
			{
				Tlv_Parse_MAC(&buf, parse_packet->u.acs.mac,  hccp_elem_len);
				break;
			}		
