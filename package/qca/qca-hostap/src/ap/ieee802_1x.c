				   ...
				   struct sta_info *sta, int authorized)
{
	...
	if (authorized) {
		...
		if(sta->roam_flag){
			...
			send_msg_to_arpp(hapd, sta, STA_ADD);
		}
	} else {
					  ...
					  struct sta_info *sta)
{
	...
	case EAP_CODE_SUCCESS:
	    ...
	    wpa_printf(MSG_INFO,"_GOLSOH_" "STA " MACSTR " - EAP Success ",MAC2STR(sta->addr));
		break;
	case EAP_CODE_FAILURE:
		os_strlcpy(buf, "EAP Failure", sizeof(buf));
		wpa_printf(MSG_INFO,"_GOLSOH_" "STA " MACSTR " - EAP Failure ",MAC2STR(sta->addr));
		break;
	default:
				...
				struct sta_info *sta, int success)
{
	...
	if (success && key && len >= PMK_LEN &&
	    ...
	    if (entry == NULL)
		    return;
		os_memcpy(entry->pmk, key, PMK_LEN);
		wpa_hexdump(MSG_DEBUG, "##RSN: entry->pmk", entry->pmk, PMK_LEN);
		os_memcpy(entry->spa, sm->addr, 6);
	    entry->pmk_len = PMK_LEN;
		os_get_time(&now);
	    entry->expiration = now.sec;
		entry->expiration += dot11RSNAConfigPMKLifetime;
	    entry->akmp = sm->wpa_key_mgmt;
	    rsn_pmkid(key, PMK_LEN, sm->wpa_auth->addr, sm->addr, entry->pmkid,
		        wpa_key_mgmt_sha256(sm->wpa_key_mgmt));
		wpa_hexdump(MSG_DEBUG, "##RSN: entry->pmkid", entry->pmkid, PMKID_LEN);
		//pmksa_cache_from_eapol_data(entry, sta->eapol_sm);
		send_pmksa_okc_info(hapd, entry);
		os_free(entry);
	}
