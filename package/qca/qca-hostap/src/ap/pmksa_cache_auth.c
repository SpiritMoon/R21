				   ...
				   struct rsn_pmksa_cache_entry *entry)
{
    ...
    wpa_printf(MSG_DEBUG,"###NOW IN FUNC pmksa_cache_free_entry");
	pmksa->pmksa_count--;
...
struct rsn_pmksa_cache_entry * pmksa_cache_get_okc(
	...
	const u8 *pmkid)
{
	wpa_printf(MSG_DEBUG,"### NOW IN FUN pmksa_cache_get_okc###");
	struct rsn_pmksa_cache_entry *entry;
	u8 new_pmkid[PMKID_LEN];
	wpa_hexdump(MSG_DEBUG,"###pmkid:",pmkid,16);
	entry = pmksa->pmksa;
	while (entry) {
		wpa_hexdump(MSG_DEBUG,"###entry->pmkid:",entry->pmkid,16);
		wpa_hexdump(MSG_DEBUG,"###entry->pmk:",entry->pmk,32);
		if (os_memcmp(entry->spa, spa, ETH_ALEN) != 0){
			entry = entry->next;
			continue;
		}
		rsn_pmkid(entry->pmk, PMK_LEN, aa, spa, new_pmkid,
			  wpa_key_mgmt_sha256(entry->akmp));
		wpa_hexdump(MSG_DEBUG,"###new_pmkid:",new_pmkid,16);
