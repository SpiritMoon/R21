...
#include <libnl/linux/rtnetlink.h>
					...
					unsigned groups, struct nlmsghdr *answer, rtnl_filter_route_t junk, void *jarg)
{
	...
	while (1) {
		...
		if (status < 0) {
			if (errno == EINTR)
				continue;
			//eag_log_info("OVERRUN");
			continue;
		}
		if (status == 0) {
			eag_log_info("EOF on netlink\n");
			return -1;
		}
							...
							const char *ip)
{
	...
	parse_rtattr(tb, NDA_MAX, RTM_RTA(r), n->nlmsg_len - NLMSG_LENGTH(sizeof(*r)));
...
eag_ipinfo_get(char * const intf, size_t n, unsigned char *const mac, uint32_t ip)
{
 ...
 uint32_t eag_get_nasip_by_ifname(const char* Interface)
{  
    
    if(!Interface) return 0;

    if(0 == strlen(Interface)) return 0;
    ...
    if(0 != ioctl(ipinfo_sockfd, SIOCGIFADDR, &ifr)) {
        //close(s);
        return 0;
    }
...
{
	if (ipinfo_sockfd >= 0) {
		eag_log_info("eag_ipinfo_init already start fd(%d)", 
			ipinfo_sockfd);
		return EAG_RETURN_OK;
	}
