...
extern unsigned int g_spectral_alarm_time;
            ...
            if (FD_ISSET(fd, &read_fds)) {
                if (fd == pnlinfo->sock_fd) {
                    if (icm_handle_spectral_data(pdev, picm) == FAILURE) {
						...
						/*
						 *drm_log(DRM_LOG_WRITE_TO_SYSLOG,"send ICM_EVENT_SPECTRAL_SWEEP_DONE to SPECTRAL_LOOP");
						 *icm_send_event(pdev,
						 *        ICM_EVENT_RECIPIENT_SPECTRAL_LOOP,
...
int icm_do_spectral_scan(ICM_INFO_T* picm, bool restore_channel)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"icm_do_spectral_scan\n");
    drm_log(DRM_LOG_NORMAL,"ACS_CONTROL:start spectral scanning, pause background scan module.\n");
    system("bg-s -x pause_scanning=1");
    system("wlanset dcm wifi0 set_util_timer_enable 0");
    system("wlanset dcm wifi1 set_util_timer_enable 0");
    if (picm->band != ICM_BAND_2_4G) {
        /* Not currently supported */
        err("Only 2.4 GHz Band currently supported for Spectral scan.");
        system("bg-s -x pause_scanning=0");    
        system("wlanset dcm wifi0 set_util_timer_enable 1");
        system("wlanset dcm wifi1 set_util_timer_enable 1");
        return FAILURE;
    }
    ...
    system("wlanset dcm wifi0 set_util_timer_enable 1");
    system("wlanset dcm wifi1 set_util_timer_enable 1");
    drm_log(DRM_LOG_NORMAL,"ACS_CONTROL: spectral scan finished, restart background scanning.\n");
    return SUCCESS;
}
...
int icm_get_channel_loading(ICM_INFO_T* picm)
{
	...
	drm_log(DRM_LOG_WRITE_TO_SYSLOG,"icm_get_channel_loading\n");
    ICM_SPECTRAL_INFO_T* psinfo = &picm->sinfo;
    ICM_DEV_INFO_T* pdev = get_pdev();
    ICM_NLSOCK_T *pnlinfo = ICM_GET_ADDR_OF_NLSOCK_INFO(pdev);
