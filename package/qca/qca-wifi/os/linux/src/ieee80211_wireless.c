...
#define TU_TO_MS(_tu)        (((_tu) * 1024) / 1000)
		...
		if (scan_num_channels || (req.min_channel_time && req.max_channel_time)) {
#else
    ...
    struct iw_point *data, char *extra)
{
    ...
    */
    ...
    if((E2BIG == res) && (data->length >= 0xffff)){
        return 0;
    }
#endif
...
#undef IBSSDFS_RECOVER_MASK
   ...
   break;
    ...
    case IEEE80211_PARAM_BSS_CHAN_INFO:
        ...
        break;
#if ATOPT_BACKGROUND_SCANNING
	//zhaoyang modifies for background scanning 2016-03-21
...
get_scan_space(void *arg, wlan_scan_entry_t se)
{
	...
	#if ATOPT_ORI_ATHEROS_BUG
    size_t se_space;
    se_space = scan_space(se, &ielen);
    if((req->space + se_space) > 0xffff){
        return 1;
    }
    else{
        req->space += se_space;
...
ieee80211_ioctl_han_priv(struct net_device *dev, struct iwreq *iwr)
{
	...
	/*End:pengdecai for han private wmm*/
		...
		case HAN_IOCTL_PRIV_DCM:
			error = ieee80211_dcm_ioctl(dev,&a,iwr);
			break;
#endif
	   /*Begin:pengdecai for han private wmm*/ 
#if ATOPT_IGMP_SNP
	   ...
	   /*End:pengdecai for han private wmm*/
		...
		case HAN_IOCTL_PRIV_TRAFFIC_LIMIT:
			error = ieee80211_ioctl_han_traffic_limit(dev, iwr);
			break;
#endif
...
static const struct iw_priv_args ieee80211_priv_args[] = {
   ...
   { IEEE80211_PARAM_GET_ACS,
	...
	{ IEEE80211_PARAM_MONITOR,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "vif_monitor" },
    { IEEE80211_PARAM_MONITOR, 0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_vif_monitor" },
#endif
};
