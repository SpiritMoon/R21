...
/*Added-End:pengdecai for 11ac station timeout*/
	...
	if (pdev->host_80211_enable) {
        ...
        if (peer) {
            ...
            if (vap) {
                ...
                if (ni) {
                    ...
                    ni->ni_stats.ns_rx_bytes += adf_nbuf_len(msdu);
                    ieee80211_free_node(ni);
                }

                mac_stats = peer->bss_peer ? &vap->iv_multicast_stats : &vap->iv_unicast_stats;
                mac_stats->ims_rx_packets++;
                mac_stats->ims_rx_bytes += adf_nbuf_len(msdu);
                mac_stats->ims_rx_data_packets++;
                mac_stats->ims_rx_data_bytes++;
                mac_stats->ims_rx_datapyld_bytes = mac_stats->ims_rx_data_bytes - (mac_stats->ims_rx_data_packets * ETHERNET_HDR_LEN);
        ...
        scn->scn_stats.rx_bytes += adf_nbuf_len(msdu);
#endif /* ATOPT_ORI_ATHEROS_BUG_11AC_COUNT */
        /* End 11ac statistics */
