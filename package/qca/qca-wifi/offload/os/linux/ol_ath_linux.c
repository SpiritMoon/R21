...
extern int han_config_chan_util_timer_enable(struct ieee80211com *ic,
	                 struct han_ioctl_priv_args *a,
	                 struct iwreq *iwr);
extern int han_dcm_ioctl(struct ieee80211com *ic,
	                 struct han_ioctl_priv_args *a,
	                 struct iwreq *iwr);
#endif
...
ol_ath_ioctl_han_priv(struct ol_ath_softc_net80211 *scn, struct iwreq *iwr)
{
	...
	switch (a.type) {
#if ATOPT_DCM
		case HAN_IOCTL_PRIV_UTIL_TIMER_ENBLE:
			error = han_config_chan_util_timer_enable(&scn->sc_ic,&a,iwr);
		break;
		case HAN_IOCTL_PRIV_DCM:
			error = han_dcm_ioctl(&scn->sc_ic,&a,iwr);
		break;
#endif
		default:
			return -EFAULT;
...
ath_getstats(struct net_device *dev)
{
    ...
    /* For 11ac statistics, Yuyaowen added. */
#if ATOPT_ORI_ATHEROS_BUG_11AC_COUNT
    stats->tx_packets = scn->scn_stats.tx_packets;
    stats->tx_bytes = scn->scn_stats.tx_bytes;
    stats->rx_packets = scn->scn_stats.rx_packets;
    stats->rx_bytes = scn->scn_stats.rx_bytes;
    stats->rx_dropped = scn->scn_stats.rx_badcrypt + scn->scn_stats.rx_badmic;
#endif /* ATOPT_ORI_ATHEROS_BUG_11AC_COUNT */
    /* End 11ac statistics. */
